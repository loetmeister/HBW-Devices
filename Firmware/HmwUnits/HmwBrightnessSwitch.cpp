   ...
   count( 0 )
{
   ...
   currentValue = 255;   // start "disabled" and wait until data arrives
   stateFlags.byte = 0;
}
...
void HmwBrightnessSwitch::set( uint8_t length, uint8_t const* const data )
{
    ...
    else if ( length == 1 && data[0] <= 200 )  // allow to set brightness value manually. Set calculation config to MANUAL_VALUE to keep value
    {
        currentValue = data[0];
    }
}
...
{
   if ( !config->getInterval() )  // sample interval == 0 disables the channel
   {
      currentValue = 255;  // also tell HmwBrightnessKey this channel is disabled
      return;
   }
   ...
   {
      stateFlags.element.blockingTimeActive = false;
      nextActionDelay = config->getInterval();    // restore configured measurement interval
      return;
   }
   ...
   {
      case Config::Behaviour::LOWEST_SAMPLE:
      {
         uint8_t _index = count;
         uint8_t idxLowest = 0;
         while (_index) {
            _index--;
            if (buffer[_index] <= buffer[idxLowest]) idxLowest = _index;
         }
         currentValue = buffer[idxLowest];
         break;
      }
      case Config::Behaviour::AVERAGE:
         if (result_sum > 0)
            currentValue = result_sum / count;  // calculate average
         else
            currentValue = 0;
      break;
      case Config::Behaviour::LAST_SAMPLE:
         currentValue = buffer[index];
      break;
      case Config::Behaviour::MANUAL_VALUE: // do not overwrite currentValue, manually set
      break;
   }
...
void HmwBrightnessSwitch::checkConfig()
{
   ...
   nextActionDelay = config->getInterval();    // nextActionDelay * 100 in main loop!
}
